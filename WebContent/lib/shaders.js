var Shaders = Backbone.Model.extend({
	...
	getRenderShader_4Byte : function(gl) {							
		var str =  "precision highp float;							\
			...
			uniform highp float u_xBinPixWd, u_yBinPixWd;												\
			uniform highp float u_visTileWd, u_visTileHt, u_canvasWd, u_plotsHt, u_zm, u_visibleTileWd, u_visibleTileHt;					\
			...
			void main() {									\
				...
				if (u_visType == 0.0) {	\
					...
					if (mod(x, u_xBinPixWd) <= u_histGap/2.0 ||  mod (x, u_xBinPixWd)  >= u_xBinPixWd - u_histGap/2.0)						\
						discard;	\
					highp float xBin = floor((x + u_pixOffsets[0] - u_containerPos[0])/u_xBinPixWd);			\
					...
					if (mod(y, u_yBinPixWd) <= u_histGap/2.0 ||  mod (y, u_yBinPixWd )  >= u_yBinPixWd - u_histGap/2.0)						\
						discard;	\
					highp float bin = floor((y + u_pixOffsets[1] - u_containerPos[1]) / u_yBinPixWd);	\
					...
					highp float xBin = floor((x + u_pixOffsets[0] - u_containerPos[0])* (u_binCnts[0]-1.0) / u_visTileWd);			\
					highp float yBin = u_yIsTopBtm ? floor((y + u_pixOffsets[1] - u_containerPos[1]) * (u_binCnts[1] - 1.0) / u_visibleTileHt) :	\
					...
					if (x < u_containerPos[0] || x > u_containerPos[0] + u_visibleTileWd || y < u_containerPos[1]  || y > u_containerPos[1] + u_visibleTileHt) discard;\
					highp float xBin = floor((x + u_pixOffsets[0] - u_containerPos[0]) * (u_binCnts[0]-1.0) / u_visTileWd);			\
					highp float yBin = floor((y + u_pixOffsets[1] - u_containerPos[1]) * (u_binCnts[1]-1.0) / u_visTileHt);	\
	...
	getQueryShader3D_2Bytes : function(gl) {
		var str =  "precision highp float;							\
			...
			float calculateSumHist(float binIdx, float col, float tileIdx){								\
				...
				for (highp float i = 0.0; i < 260.0; i++){							\
					ir = i + alphaLo;													\
					if (ir > alphaHi) break;											\
					for (highp float j = 0.0; j < 260.0; j++){						\
						jr = j + betaLo;												\
						if (jr > betaHi)		break;									\
						texIdx = start + ir*alphaOffset + jr*betaOffset; \
						fz = floor(texIdx/2.0);						\
