var VisManager = Backbone.Model.extend({
	...
	fireEvent : function(imEvt) {
		...
		if (logging)
			this.logImEvt(imEvt);
		//var evtQueue = this.get("imEvts");
		//this.set("imEvts", evtQueue.concat([imEvt]));
		//this.set("imEvts", [imEvt]);
		// if (!visManager.get("timerId")){
		// 	// evtQueue = [imEvt];
		// 	var timerId = setInterval(function(){
		// 		if (visManager.get("imEvts").length > 0)
		// 			visManager.get("activeWkSheet").processEvent(visManager.get("imEvts").shift());
		// 		if (visManager.get("imEvts").length == 0){
		// 			clearInterval(visManager.get("timerId"));
		// 			visManager.set("timerId", undefined);
		// 		}
		// 	}, delay);
		// 	visManager.set("timerId", timerId);
		// }
	},

	logImEvt : function(imEvt){
		var type,spec,value="";
		switch (imEvt.getType()) {
		...
		case ImMensEvent.evtTypes.brush:
			type = "brush";
			spec = imEvt.getSource().toString();
			for (var col in imEvt.getValue())
				value += imEvt.getValue()[col].toString()+"_";
			break;
		...
		var param = "dataset=" + currentDataSet + "&delay=" + delay + "&type=imMensEvt&subject=" + participant + "&time=" + Date.now() + "&action=" + type + "&spec=" + spec + "&value=" + value;
		logger.httpPost(param);
	},
